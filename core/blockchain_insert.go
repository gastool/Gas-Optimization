...
func newInsertIterator(chain types.Blocks, results <-chan error, validator Validator, config *typing.MonitorConfig) *insertIterator {
...
func (it *insertIterator) next() (*types.Block, error) {
	...
	// for debugging
	//if it.chain[it.index].NumberU64() >= 430000 {
	//	a := 1
	//	_ = a
	//}

	if it.MonitorConfig.ReplaceMonitoredContractCode {
		currentBlock := it.chain[it.index]

		// record the original block hash
		util.SaveOriginalBlockHash(currentBlock.NumberU64(), currentBlock.Hash())

		// fake parent hash
		if it.index > 0 {
			currentBlock.HeaderNoCopy().ParentHash = it.chain[it.index - 1].Hash()
		} else {
			parentBlockHash, exist := util.LoadTaintedBlockHash(currentBlock.NumberU64() - 1)
			if exist {
				currentBlock.HeaderNoCopy().ParentHash = parentBlockHash
			}
		}

		// fake uncles and uncleHash
		if len(currentBlock.Uncles()) > 0 {
			firstBlockNumber := it.chain[0].Number().Int64()
			var updatedUncleHeaders []*types.Header
			for _, uncleHeader := range currentBlock.Uncles() {
				uncleParentIndex := uncleHeader.Number.Int64() - firstBlockNumber - 1
				if uncleParentIndex >= 0 {
					uncleHeader.ParentHash = it.chain[uncleParentIndex].Hash()
				} else {
					uncleParentBlockHash, exist := util.LoadTaintedBlockHash(uncleHeader.Number.Uint64() - 1)
					if exist {
						uncleHeader.ParentHash = uncleParentBlockHash
					}
				}
				updatedUncleHeaders = append(updatedUncleHeaders, uncleHeader)
			}
			currentBlock.SetUncles(updatedUncleHeaders)
			currentBlock.SetUncleHash(types.CalcUncleHash(updatedUncleHeaders))
		}
	}
