...
func opExtCodeSize(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error) {
	slot := callContext.stack.peek()
	interpreter.evm.ReadeeAddresses[util.ConvertAddressToLowerCaseString(slot.Bytes20())] = struct{}{}
...
func opExtCodeCopy(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error) {
	...
	interpreter.evm.ReadeeAddresses[util.ConvertAddressToLowerCaseString(addr)] = struct{}{}
...
func opExtCodeHash(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error) {
	slot := callContext.stack.peek()
	address := common.Address(slot.Bytes20())
	interpreter.evm.ReadeeAddresses[util.ConvertAddressToLowerCaseString(address)] = struct{}{}
...
func opBlockhash(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error) {
	...
	if num64 >= lower && num64 < upper {
		if interpreter.evm.MonitorConfig.ReplaceMonitoredContractCode {
			realBlockHash, exist := util.LoadOriginalBlockHash(num64)
			if !exist {
				panic("Blockhash not found for block " + strconv.FormatUint(num64, 10))
			}
			num.SetBytes(realBlockHash.Bytes())
		} else {
...
func opSload(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error) {
	atomic.AddUint64(&interpreter.evm.SLoadCount, 1)
...
func opSstore(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error) {
	atomic.AddUint64(&interpreter.evm.SStoreCount, 1)
...
func opGas(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error) {
	manipulatedGas := callContext.contract.Gas
	evm := interpreter.evm

	// ignore prefetch environment
	if !evm.IsPrefetch {
		callChainGasOpIndexesString := util.EncodeCallChainIndexes(evm.CallChainGasOpIndexes)

		//if evm.Context.BlockNumber.Uint64() == 607503 && evm.StateDB.TxIndex() == 7 {
		//	a := 1
		//	_ = a
		//}
		...
		if !evm.MonitorConfig.ReplaceMonitoredContractCode {
			// record the gas
			if _, exist := evm.CallChainOriginalGasSiteInfo[callChainGasOpIndexesString]; exist {
				panic("CallChainOriginalGasSiteInfo value already set, txHash = " + evm.TransactionHash.String() +
					", callChainIndexesWithGas = " + callChainGasOpIndexesString)
			}
			evm.CallChainOriginalGasSiteInfo[callChainGasOpIndexesString] = typing.GasSiteInfo{
				TransientGas:  callContext.contract.Gas,
			...
			callLocator := &typing.CallLocator{
				...
				CallChainIndexes: callChainGasOpIndexesString,
			}
			evm.MonitorConfig.CallChainOriginalGasSiteInfoLock.Lock()
			gasSiteInfo, exist := evm.MonitorConfig.CallChainOriginalGasSiteInfo[*callLocator]
			if evm.MonitorConfig.DependencyInfoGC {
				evm.MonitorConfig.CallChainOriginalGasSiteInfoGcList = append(evm.MonitorConfig.CallChainOriginalGasSiteInfoGcList, callLocator) // add to gc list
			}
			evm.MonitorConfig.CallChainOriginalGasSiteInfoLock.Unlock()
			if !exist {
				panic("CallChainOriginalGasSiteInfo missing, txHash = " + evm.TransactionHash.String() +
					", callChainGasOpIndexesString = " + callChainGasOpIndexesString)
			}
			if gasSiteInfo.ToAddressTail != evm.LastCallInfo.ToAddressTail ||
				gasSiteInfo.Value != evm.LastCallInfo.Value ||
				gasSiteInfo.Type != evm.LastCallInfo.Type {
				panic("newGasSiteInfo != oldGasSiteInfo" +
					", newGasSiteInfo.ToAddressTail = " + evm.LastCallInfo.ToAddressTail + ", oldGasSiteInfo.ToAddressTail = " + gasSiteInfo.ToAddressTail +
					", newGasSiteInfo.Value = " + evm.LastCallInfo.Value + ", oldGasSiteInfo.Value = " + gasSiteInfo.Value +
					", newGasSiteInfo.Type = " + evm.LastCallInfo.Type.ReadableString() + ", oldGasSiteInfo.Type = " + gasSiteInfo.Type.ReadableString())
			}
			if manipulatedGas != gasSiteInfo.TransientGas {
				manipulatedGas = gasSiteInfo.TransientGas
			}
		}
	}

	callContext.stack.push(new(uint256.Int).SetUint64(manipulatedGas))
...
func opCreate(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error) {
	...
	InjectCallInfoToEvm(interpreter.evm, nil, bigVal, typing.Create)

	res, addr, returnGas, suberr := interpreter.evm.Create(callContext.contract, input, gas, bigVal)

	ClearCallInfoInEvm(interpreter.evm)
...
func opCreate2(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error) {
	...
	InjectCallInfoToEvm(interpreter.evm, nil, bigEndowment, typing.Create2)
...
func opCall(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error) {
	...
	interpreter.evm.CalleeAddresses[util.ConvertAddressToLowerCaseString(toAddr)] = struct{}{}
	InjectCallInfoToEvm(interpreter.evm, &toAddr, bigVal, typing.Call)
...
func opCallCode(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error) {
	...
	interpreter.evm.CalleeAddresses[util.ConvertAddressToLowerCaseString(toAddr)] = struct{}{}
	InjectCallInfoToEvm(interpreter.evm, &toAddr, bigVal, typing.CallCode)
...
func opDelegateCall(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error) {
	...
	interpreter.evm.CalleeAddresses[util.ConvertAddressToLowerCaseString(toAddr)] = struct{}{}
	InjectCallInfoToEvm(interpreter.evm, &toAddr, &big.Int{}, typing.DelegateCall)
...
func opStaticCall(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error) {
	...
	interpreter.evm.CalleeAddresses[util.ConvertAddressToLowerCaseString(toAddr)] = struct{}{}
	InjectCallInfoToEvm(interpreter.evm, &toAddr, &big.Int{}, typing.StaticCall)
...
func InjectCallInfoToEvm(evm *EVM, toAddress *common.Address, value *big.Int, callType typing.CallType) {
	evm.LastCallInfo.Type = callType
	evm.LastCallInfo.Value = value.String()
	var toAddressString string
	if toAddress == nil {
		toAddressString = ""
	} else {
		toAddressString = toAddress.String()
		toAddressString = toAddressString[len(toAddressString) - 6:]
	}
	evm.LastCallInfo.ToAddressTail = toAddressString
}

func ClearCallInfoInEvm(evm *EVM) {
	evm.LastCallInfo.Type = typing.Unknown
	evm.LastCallInfo.Value = "0"
	evm.LastCallInfo.ToAddressTail = ""
}
